	...
	export default {
		...
		methods: {
			...
			onFocusHandler(e) {
				...
				if (this.valueNumber === "" || this.valueNumber === null || (this.valueNumber === 0 && this.clearZero)) {
					this.amount = null;
				} else {
			...
			process(value) {
				...
				if (!this.minus && value < 0) {
					this.min >= 0 ? this.update(this.min) : this.update(null);
					return true;
				}
			},
			 ...
			 */
			format(value) {
				if ((this.emptyValue === "" || this.emptyValue === null) && (value === null || value === "")) {
					return null;
				}

				return accounting.formatMoney(value === null || value === "" ? this.emptyValue : value, {
			...
			unformat(value) {
				...
				return accounting.unformat(value, this.decimalSeparatorSymbol);
			},
		}
	}
</script>
